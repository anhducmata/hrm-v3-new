        ...
        <nav id="topbar" role="navigation" style="margin-bottom: 0; z-index: 2;"
            ...
            <div class="topbar-main"><a id="menu-toggle" href="#" class="hidden-xs"><i class="fa fa-bars"></i></a>
                ...
                <ul class="nav navbar navbar-top-links navbar-right mbn">
                    <li class="dropdown" id="btn_message"><a data-hover="dropdown" style="cursor: pointer;" class="dropdown-toggle" aria-expanded="false"><i
                            class="fa fa-bell fa-fw"></i><span class="badge badge-green count" id="count_noti"></span></a>
                        <ul class="dropdown-menu dropdown-alerts">
                            <li id="count_message"></li>
                            <li>
                                <div class="dropdown-slimscroll">
                                    <ul class="dropdown-menu-item" id="noti">
                                        @if(isset($getNoti))
                                        @foreach($getNoti as $noti)
                                            <li><a href="detail/{{ $noti->noti_id }}">{{ $noti->title }}</a></li>
                                        @endforeach
                                        @endif
                                    </ul>
                                </div>
...
$(document).ready(function(){
    ...
    //Lay noti chua doc khi reload page
    $.ajax({
        url: "getNoti",
        type: "POST",
        data:   {getNoti: 'get Noti'},
        success: function(data){
            var html = '';
            for(i = 0; i < data.length; i++){
                html += '<li><a href=detail/'+ data[i]['noti_id']+ '><span class="label label-blue">'+ data[i]['title']+'</span></a></li>';
            ...
            $("#noti").html(html);
             ...
             //moi login vao ma chua xem
            var count_message_login = $('#noti li').length;
            if(count_message_login > 0){
             $('#count_message').html('<p>You have ' + count_message_login + ' message</p>');
             $('#count_noti').html(count_message_login);
            }
        },
        error: function(msg){
            alert("fail" + ' ' + msg);
    ...
    $("#btn_create_noti").on('click',(function(evt){           
        evt.preventDefault();
        $.ajaxSetup({
          headers: {
            'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')
        ...
        $.ajax({
            ...
            success: function(data){
                console.log(data);
                socket.emit("sendmessage", {'title' : $('#title').val(), 'message' : $('#message').val(), 'id' : data.id, 'check_user' : data.check_user});
     ...
     socket.on("server-send", function(data){
        $('#noti').append('<li><a href=detail/' + data.id + 'target="_blank"><span class="label label-blue">' + data.title +'</span></a></li>');
        ...
        $.ajaxSetup({
         ...
         $.ajax({
            ...
            success: function(data){
                console.log(data);
                $('#count_message').html('');
                $('#count_message').html('<p>You have ' + data + ' message</p>');
                $('#count_noti').html('');
                $('#count_noti').html(data);
            },
            error: function(data){
                console.log("data");
            }
